...
2. **Effect of clipping.** Re-run the hardened recipe with `--max-grad-norm 0.0`. On Tiny Shakespeare with this small a model, clipping rarely fires (the gradients aren't dramatic). On a real-text corpus in Ch.28 it does, and you'll see the difference.
